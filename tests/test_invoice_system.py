...
def create_invoice():
    res = client.post("/invoices/", json=SAMPLE_INVOICE)
...
def test_create_invoice_unique_ids():
    id1 = client.post("/invoices/", json=SAMPLE_INVOICE).json()["invoice_id"]
    id2 = client.post("/invoices/", json=SAMPLE_INVOICE).json()["invoice_id"]
    assert id1 != id2
...
def test_get_invoice_format_json():
    invoice_id = create_invoice()
    res = client.get(f"/invoices/{invoice_id}?format=json")
    assert res.status_code == 200
    assert res.json()["client_name"] == "Test Client"

def test_get_invoice_format_ubl():
    invoice_id = create_invoice()
    res = client.get(f"/invoices/{invoice_id}?format=ubl")
    assert res.status_code == 200
...
def test_get_invoice_format_xml():
    invoice_id = create_invoice()
    res = client.get(f"/invoices/{invoice_id}?format=xml")
    assert res.status_code == 200
    assert "application/xml" in res.headers["content-type"]
...
def test_get_invoice_format_csv():
    invoice_id = create_invoice()
    res = client.get(f"/invoices/{invoice_id}?format=csv")
    assert res.status_code == 200
...
def test_get_invoice_format_pdf():
    invoice_id = create_invoice()
    res = client.get(f"/invoices/{invoice_id}?format=pdf")
    assert res.status_code == 200
    assert res.headers["content-type"] == "application/pdf"
    assert res.content[:4] == b"%PDF"

def test_get_invoice_not_found():
    res = client.get("/invoices/does-not-exist")
    assert res.status_code == 404

def test_get_invoice_not_found_ubl():
    res = client.get("/invoices/does-not-exist?format=ubl")
    assert res.status_code == 404

def test_get_invoice_not_found_csv():
    res = client.get("/invoices/does-not-exist?format=csv")
    assert res.status_code == 404

def test_get_invoice_not_found_pdf():
    res = client.get("/invoices/does-not-exist?format=pdf")
    assert res.status_code == 404
...
def test_update_invoice_name():
    invoice_id = create_invoice()
    res = client.put(f"/invoices/{invoice_id}", json={"client_name": "Updated"})
    assert res.status_code == 200
    assert res.json()["client_name"] == "Updated"

def test_update_invoice_email():
    invoice_id = create_invoice()
    res = client.put(f"/invoices/{invoice_id}", json={"client_email": "new@email.com"})
    assert res.status_code == 200
    assert res.json()["client_email"] == "new@email.com"

def test_update_invoice_not_found():
    res = client.put("/invoices/does-not-exist", json={"client_name": "Ghost"})
    assert res.status_code == 404
...
def test_delete_invoice():
    invoice_id = create_invoice()
    res = client.delete(f"/invoices/{invoice_id}")
    assert res.status_code == 200
...
def test_delete_invoice_not_found():
    res = client.delete("/invoices/does-not-exist")
    assert res.status_code == 404
